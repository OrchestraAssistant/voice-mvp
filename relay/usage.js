...
/** The rate names an entry may set. Anything else in a `perMillion` block is a
 *  typo, and a typo here silently prices something at zero -- the exact
 *  failure the unpriced-model rule exists to prevent, arriving by another
 *  door. So it is rejected rather than ignored. */
export const RATE_KEYS = ["inputText", "inputAudio", "cachedText", "cachedAudio", "outputText", "outputAudio"];
const ENTRY_KEYS = ["provider", "perMillion", "transcriptionPerMinute", "audioPerMinute", "note"];

export const PRICES_FILE = process.env.VOICE_PRICES || path.join(__dirname, "prices.yaml");

/**
 * Read a price table.
 *
 * Config rather than source, because rates change on someone else's schedule
 * and re-editing a module to track them is how a table ends up stale and
 * trusted. YAML for the comments: every rate wants a date and a link next to
 * it, and that is the whole argument against JSON here. A .json file is read
 * too, for a table generated by something else.
 *
 * Validates rather than trusting. Every value that reaches the arithmetic has
 * to be a number, and every key has to be one this code actually reads.
 */
export function loadPrices(file = PRICES_FILE) {
  const raw = fs.readFileSync(file, "utf8");
  const doc = file.endsWith(".json") ? JSON.parse(raw) : yaml.load(raw);
  // Two accepted shapes: the annotated file, and a bare map of models, which
  // is what a hand-written override or a generated table tends to look like.
  const models = doc?.models ?? doc;
  if (!models || typeof models !== "object") throw new Error(`${file}: no models in this price table`);

  const table = {};
  for (const [model, entry] of Object.entries(models)) {
    if (!entry || typeof entry !== "object") throw new Error(`${file}: ${model} is not a table of rates`);
    for (const key of Object.keys(entry)) {
      if (!ENTRY_KEYS.includes(key)) {
        throw new Error(`${file}: ${model} has an unknown field "${key}" (expected ${ENTRY_KEYS.join(", ")})`);
      }
    }
    for (const key of Object.keys(entry.perMillion ?? {})) {
      if (!RATE_KEYS.includes(key)) {
        throw new Error(`${file}: ${model}.perMillion has an unknown rate "${key}" (expected ${RATE_KEYS.join(", ")})`);
      }
    }
    for (const [where, value] of [
      ...Object.entries(entry.perMillion ?? {}),
      ["transcriptionPerMinute", entry.transcriptionPerMinute],
      ["audioPerMinute", entry.audioPerMinute],
    ]) {
      if (value == null) continue;
      if (typeof value !== "number" || !Number.isFinite(value) || value < 0) {
        throw new Error(`${file}: ${model}.${where} is ${JSON.stringify(value)}, which is not a rate`);
      }
    }
    table[model] = entry;
  }
  table[CHECKED] = doc?.checked ?? null;
  return table;
}

/** When the loaded table was last checked against a provider's own page.
 *  A symbol so it cannot collide with a model called "checked". */
export const CHECKED = Symbol.for("voice.prices.checked");

// Loaded once, lazily, so importing this module for the arithmetic alone --
// which the tests do -- never depends on a file being present or valid.
let cached = null;
export function prices() {
  ...
  try {
    ...
    // Fails soft, and only here. A missing or broken price file must not stop
    // a session being measured: the token counts are the durable part and the
    // rates are not, so everything reports unpriced and the summary still
    // lands. `loadPrices` itself still throws, so `--prices` fails loudly.
    console.error(`Price table not loaded (${err.message}); sessions will report unpriced.`);
    cached = {};
  }
  return cached;
}
 ...
 */
export function priceSession(tally, table = prices()) {
  const entry = table[tally.model];
...
export function summaryEvent(tally, table = prices()) {
  const priced = priceSession(tally, table);
  return {
    type: "summary",
    model: tally.model,
